...
datagen<-function(seed=sample(1:1000000,size=1),ssize=500,popsize=1500000,OR_C=3,OR_WI=1,OR_WC=5,OR_H=1.5,em=0,cfV0=F,cfV1=F,return_full=F){
  ...
  #Infected with COVID
  Infec_COVID<- rbinom(prob=plogis( -log(OR_C)*V -5 + 2*C - 0.25*exp(C)+log(3)*U2*(1.5-V)-2*U1), size=1,n=popsize) #0.009
  ...
  W[Infec_COVID==1]<-rbinom(prob=plogis(-2.25+1*C[Infec_COVID==1]-log(OR_WC)*V[Infec_COVID==1]-1*U1[Infec_COVID==1]+0.5*U2[Infec_COVID==1]*(1-V[Infec_COVID==1])),size=1, n=sum(Infec_COVID))
...
for (i in 1:1000){
  ...
  write(c(i,est1,CI1, est2,CI2, est3,CI3,CI3.2,est4,CI4,CI4.2, est5,CI5,CI5.2, 
          est6,CI6,CI6.2, est7,CI7,CI7.2,est8,CI8, CI8.2,est9,CI9, CI9.2,est10,CI10,CI10.2, est11, CI11, est12, CI12),file="Study2results1000_mrr45.txt",ncolumns=70,append=T)
}




res1<-read.table("Study2results250_mrr45.txt",header=F)
head(res1)
res1 <- na.omit(res1)
dim(res1)
#truth mRR
